...
const plugins = pluginFiles.map(file => require(path.join(pluginDirectory, file)));
...
gss.ev.on('messages.upsert', async (chatUpdate) => {
    try {
        ...
        if (mek.key.id.startsWith('FatihArridho_')) return;
            ...
            // Create a function to handle plugins
            const handlePlugins = async (m, gss) => {
            ...
            const m = smsg(gss, mek, store);
            await handlePlugins(m, gss); // Updated to pass 'm' instead of 'mek'
...
// Define an object to store poll data
const polls = {};
    ...
    for (const { key, update } of chatUpdate) {
        if (update.pollUpdates && key.fromMe) {
            ...
            if (pollCreation) {
                ...
                if (!tocommand) return;
                const prefixedPollName = `.${tocommand}`;
                ...
                gss.appenTextMessage(prefixedPollName, chatUpdate, { pollKey });
            }
        }
    }
});
...
gss.ev.on('group-participants.update', async (anu) => {
    ...
    if (connection === "close") {
        ...
        gss.sendMessage(gss.user.id, {
            text: "Welcome back! Your connection to the server is established."
        });
    }
});
